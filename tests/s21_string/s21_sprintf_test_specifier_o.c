...
START_TEST(test2) {
  char buffer1[8000] = {0};
  char buffer2[8000] = {0};
  s21_sprintf(buffer1, "I'm |%o| years old", 123456789);
  sprintf(buffer2, "I'm |%o| years old", 123456789);
  ck_assert_str_eq(buffer1, buffer2);
}
...
START_TEST(test3) {
  char buffer1[8000] = {0};
  char buffer2[8000] = {0};
  s21_sprintf(buffer1, "I'm |%lo| years old", 5124095576030430L);
  sprintf(buffer2, "I'm |%lo| years old", 5124095576030430L);
  ck_assert_str_eq(buffer1, buffer2);
}
...
START_TEST(test4) {
  char buffer1[8000] = {0};
  char buffer2[8000] = {0};
  s21_sprintf(buffer1, "I'm |%.15o| years old", 123456789);
  sprintf(buffer2, "I'm |%.15o| years old", 123456789);
  ck_assert_str_eq(buffer1, buffer2);
}

Suite *s21_sprintf_test_suite_specifier_o() {
  Suite *s = suite_create("\033[42m-=s21_sprintf test specifier o=-\033[0m");
  ...
  //   выйти за допустимые границы числа tcase_add_test(t, test8);
